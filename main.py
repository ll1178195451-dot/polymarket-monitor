...
def main():
    print("XRP SOL 监控已启动 (阈值 700 shares，显示持仓成本)")
            ...
            for mins in [5, 15]:
                ...
                for h in holders:
                    ...
                    if amount >= THRESHOLD and wallet:
                        total_cost = float(h.get("totalCost", 0))
                        avg_cost = total_cost / amount if amount > 0 else 0
                        outcome = h.get("outcome", "").upper()
                        direction = "UP" if outcome == "UP" else "DOWN"

                        alert = "Top Holders 大持仓 | 市场: " + name + " | 时间段: " + time_period + " | 方向: " + direction + " | 总持仓: " + str(int(amount)) + " shares | 持仓成本: " + str(round(avg_cost, 6)) + " | 钱包: " + wallet
                        print("[" + datetime.now().strftime('%H:%M:%S') + "] " + alert)
