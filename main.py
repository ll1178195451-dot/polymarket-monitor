...
def main():
    print("XRP SOL 5分钟和15分钟监控已启动 (测试模式：阈值 100 shares，只监控 XRP 和 SOL)")
        ...
        for asset in ["xrp", "sol"]:
            for mins in [5, 15]:
                ...
                for trade in trades:
                    ...
                    # 严格过滤：只处理 XRP 和 SOL 的交易
                    market_name = str(trade.get("market", "")).lower()
                    if asset not in market_name:
                        continue
