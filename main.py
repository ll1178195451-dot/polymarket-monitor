...
# Telegram 配置 (已填入您的 Token)
TG_BOT_TOKEN = "8627467932:AAEmtYhcoGfni9FoFtiExNv6cwtMw9XAuuY"
# Chat ID 将在您发送 /start 后自动获取并保存
TG_CHAT_ID = "" 
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if monitor:
        # 自动锁定 Chat ID
        monitor.chat_id = update.effective_chat.id
        await update.message.reply_text(f"✅ <b>已成功绑定 Chat ID:</b> <code>{monitor.chat_id}</code>\n\n" + monitor.get_status_text(), parse_mode='HTML')
    else:
